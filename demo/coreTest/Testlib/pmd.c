...
static void cust_pm_message(T_AMOPENAT_PM_MSG* pmMessage)
{
    switch(pmMessage->evtId)
    {
        case OPENAT_DRV_EVT_PM_POWERON_ON_IND:
			iot_debug_print("[coreTest-pmd]: powerOnReason: %d",pmMessage->param.poweronind.powerOnReason);
            break;
        case OPENAT_DRV_EVT_CHR_PRESENT_IND:
			iot_debug_print("[coreTest-pmd]:CHR TurnOff");
            break;
        case OPENAT_DRV_EVT_BAT_CHARGING:
			iot_debug_print("[coreTest-pmd]:BAT_CHARGING");
            break;
		case OPENAT_DRV_EVT_BAT_CHR_FULL:
			iot_debug_print("[coreTest-pmd]:BAT_CHR_FULL");
            break;
        default:
            break;
    }
...
void pwdTest(void)
{    
    iot_debug_print("[coreTest-pmd]: app_main");
