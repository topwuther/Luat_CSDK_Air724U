...
#     endif
      ...
      {
#        if PNG_ZLIB_VERNUM < 0x1240
            ret = elua_inflateReset(&png_ptr->zstream);
#        else
...
#        else
            ret = elua_inflateInit2(&png_ptr->zstream, window_bits);
#        endif
 ...
 * set Z_FINISH will be passed to the final elua_inflate() call and Z_STREAM_END must
    ...
    /* OUTPUT: */ png_bytep output, png_alloc_size_t *output_size_ptr)
{
   if (png_ptr->zowner == owner) /* Else not claimed */
   {
      ...
      do
      {
          ...
          */
         ret = elua_inflate(&png_ptr->zstream, avail_out > 0 ? Z_NO_FLUSH :
...
#  endif
   ...
   if (limit >= prefix_size + (terminate != 0))
   {
      ...
      if (ret == Z_OK)
      {
         ...
         if (ret == Z_STREAM_END)
         {
            /* Use 'elua_inflateReset' here, not 'inflateReset2' because this
             ...
             * maintained.
             */
            if (elua_inflateReset(&png_ptr->zstream) == Z_OK)
            {
            ...
            {
               /* elua_inflateReset failed, store the error message */
               png_zstream_error(png_ptr, ret);
   ...
   int finish)
{
   if (png_ptr->zowner == png_ptr->chunk_name)
   {
      ...
      do
      {
          ...
          * streams.
          */
         ret = elua_inflate(&png_ptr->zstream,
   ...
   png_alloc_size_t avail_out)
{
   ...
   do
   {
       ...
       */
      ret = elua_inflate(&png_ptr->zstream, Z_NO_FLUSH);
