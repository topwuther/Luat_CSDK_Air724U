...
png_reset_zstream(png_structrp png_ptr)
{
   ...
   return (elua_inflateReset(&png_ptr->zstream));
}
#endif /* PNG_READ_SUPPORTED */
...
} png_sRGB_checks[] =
{
    ...
    */
   /* elua_adler32, crc32, MD5[4], intent, date, length, file-name */
   ...
   png_const_bytep profile, uLong adler)
{
   ...
   for (i=0; i < (sizeof png_sRGB_checks) / (sizeof png_sRGB_checks[0]); ++i)
   {
         ...
         png_get_uint_32(profile+96) == png_sRGB_checks[i].md5[3])
      {
          ...
          * case we can only use the length and elua_adler32 (note that these
...
#        endif
         ...
         {
            /* Now calculate the elua_adler32 if not done already. */
            if (adler == 0)
            {
               adler = elua_adler32(0, NULL, 0);
               adler = elua_adler32(adler, profile, length);
            }
