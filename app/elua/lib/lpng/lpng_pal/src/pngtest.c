...
static unsigned char tempBuf[500];
...
test_one_file(PNG_CONST char *inname, PNG_CONST char *outname)
{
   ...
   result = setjmp(png_jmpbuf(read_ptr));
   //pngtest_debug("result=%d\n", result);
   fprintf(STDERR, "zhuth result=%d\n", result);
   if (result)
   {
...
#  endif
   ...
   for (pass = 0; pass < num_pass; pass++)
   {
      ...
      for (y = 0; y < height; y++)
      {
         ...
         memcpy(&tempBuf[tempLen], row_buf, png_get_rowbytes(read_ptr, read_info_ptr));
         tempLen += png_get_rowbytes(read_ptr, read_info_ptr);
...
int TestPngLib(const char *in, const char *out)
{
    return test_one_file(in, out);
}
#else
int TestPngLib(const char *in, const char *out)
{
