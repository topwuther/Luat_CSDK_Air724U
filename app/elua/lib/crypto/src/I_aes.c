...
void AES128_ECB_decrypt(const uint8_t* input, const uint8_t* key, uint8_t *output)
{
    ...
    printf("AES128_ECB_decrypt len=%d,input=%s\n",strlen(input),input);
...
#endif
#endif
static void AES_Init(uint8_t nKeyLen)
{
    if(nKeyLen==16)
    {
        AES128 = 1;
    ...
    {
        printf("unsupport aes password length:%d\n", nKeyLen*8);
    }
}
...
void AES_Encrypt(uint8_t *pMode, uint32_t nBufLen, uint8_t *pBuf, uint8_t nKeyLen, uint8_t *pKey, uint8_t *pIV)
{
    ...
    else if(strcmp(pMode,"CBC") == 0)
    {
        ...
        AES_CBC_encrypt_buffer(&ctx, pBuf, nBufLen);
    }
    else if (strcmp(pMode,"CTR") == 0)
    {
        struct AES_ctx ctx;
        AES_init_ctx_iv(&ctx, pKey, pIV);
        AES_CTR_xcrypt_buffer(&ctx, pBuf, nBufLen);
    }
}
...
void AES_Decrypt(uint8_t *pMode, uint32_t nBufLen, uint8_t *pBuf, uint8_t nKeyLen, uint8_t *pKey, uint8_t *pIV)
{
    ...
    else if(strcmp(pMode,"CBC") == 0)
    {
        ...
        AES_init_ctx_iv(&ctx, pKey, pIV);
        AES_CBC_decrypt_buffer(&ctx, pBuf, nBufLen);
    }
    else if (strcmp(pMode,"CTR") == 0)
    {
        struct AES_ctx ctx;
