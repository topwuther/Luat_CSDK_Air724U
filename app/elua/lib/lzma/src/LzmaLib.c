...
#ifdef MUXUSE_DLMALLOC_MEMORY_AS_LUA_SCRIPT_LOAD
static void *SzAlloc(void *p, size_t size) { p = p; return OPENAT_malloc(size); }
static void SzFree(void *p, void *address) { p = p; OPENAT_free(address); }
#else
...
#endif
  res = LUA_LzmaDec_DecodeToBuf(&state, dstBuf, &outProcessed, &srcBuf[LZMA_HEAD_SIZE], &inProcessed, LZMA_FINISH_ANY, &status);
#ifndef WIN32
...
decode_exit:
  LUA_LzmaDec_Free(&state, &g_Alloc);
  return res;
}
...
CLzmaDec lzmaDecState;
UInt64 lzmaUnpackSize;


SRes lzmaDecodeStreamInit(const unsigned char *inbuff, const unsigned int inlen)
{
	int i = 0;

	LUA_LzmaDec_Construct(&lzmaDecState);
	RINOK(LUA_LzmaDec_Allocate(&lzmaDecState, inbuff, LZMA_PROPS_SIZE, &g_Alloc));
	LUA_LzmaDec_Init(&lzmaDecState);

    for (i = 0; i < 8; i++)
    {
		lzmaUnpackSize += (UInt64)inbuff[LZMA_PROPS_SIZE + i] << (i * 8);
    }
...
void lzmaDecodeStreamFree()
{
	LUA_LzmaDec_Free(&lzmaDecState, &g_Alloc);
}


int LzmaDecodeStream(const unsigned char *inbuff, 
