...
int find_dm_entry( const char* name, char **pactname )
{
  int i;
...
/*+\NEW\liweiqiang\2013.5.11\目录下文件无法正常访问*/
  ...
  for( i = 0; i < dm_get_num_devices(); i ++ )
  {
    ...
    if( !strncasecmp( tempname, pdev->name, strlen(pdev->name)) )
      break;
...
int _rename_r( const char *old, const char *new)
{
    return platform_sys_file_rename(old, new);
}
