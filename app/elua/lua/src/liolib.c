...
static int io_open_dir (lua_State *L) {
  ...
  if(dirState == DIR_IDLE)
  	{
	  OPENAT_print("io_open_dir  %s",dirname);
	  dirDict = L_MALLOC(strlen(dirname)+1);
	  memset(dirDict,'\0',strlen(dirname)+1);
...
static int io_read_dir (lua_State *L) {
 int iRet; 
 char newPath[120];
 AMOPENAT_FS_FIND_DATA file_info;
 if( dirState == DIR_READY)
 {
	iRet = OPENAT_find_first_file(dirDict,&file_info);
	if(iRet != 1)
	{
		return 0;
	}
	...
	{
		dirState = DIR_READING;
	}
 }
 else if ( dirState == DIR_READING )
 {
	iRet =  OPENAT_find_next_file(1,&file_info);
	if(iRet != 1)
	{
		return 0;
	}
 ...
 if(strlen(file_info.st_name) > 0  && E_FS_ATTR_ARCHIVE == file_info.st_mode  )
 {
	 memset(newPath,0,120);
	 memcpy(newPath, dirDict, strlen(dirDict));
	 memcpy(newPath+strlen(dirDict), "/", 1);
	 memcpy(newPath+strlen(dirDict) + 1, file_info.st_name,strlen(file_info.st_name)+1);
	 file_info.st_size =  OPENAT_get_file_size(newPath);
 }

 lua_pushinteger(L, file_info.st_mode);
 lua_pushlstring(L, file_info.st_name,strlen(file_info.st_name));
 lua_pushinteger(L, file_info.st_size);
 return 3;
