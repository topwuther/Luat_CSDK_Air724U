...
int platform_adc_open(unsigned id, unsigned mode, int value)
{
	...
	switch(value)
	{
		case PLATFORM_ADC_SCALE_1V250:
			scale = OPENAT_ADC_SCALE_1V250;
			break;
		case PLATFORM_ADC_SCALE_2V444:
			scale = OPENAT_ADC_SCALE_2V444;
			break;
		case PLATFORM_ADC_SCALE_3V233:
			scale = OPENAT_ADC_SCALE_3V233;
			break;
		case PLATFORM_ADC_SCALE_DEFAULT:
		case PLATFORM_ADC_SCALE_5V000:
			scale = OPENAT_ADC_SCALE_5V000;
			break;
		default:
			return PLATFORM_ERR;
	}
...
int platform_adc_read(unsigned id, int *adc, int *volt)
{
    ...
    u16 voltage = 0xffff;
    BOOL ret;
	/*+\bug3689\zhuwangbin\2020.11.25\adc添加可选参数scale， 修改warning*/
...
int platform_pwm_set(unsigned id, int param0, int param1)
{
	...
	switch(id)
	{
		...
		{
			pwmcfg.port = OPENAT_PWM_PWL_OUT1;
    		pwmcfg.cfg.pwt.freq = param0;
    		pwmcfg.cfg.pwt.level = param1;
		}
		break;
