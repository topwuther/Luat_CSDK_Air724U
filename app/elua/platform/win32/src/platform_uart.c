...
void send_uart_message(int uart_id)
{
    ...
    PlatformMessage *pMsg = L_MALLOC(sizeof(PlatformMessage));
...
void simulate_uart_thread(LPVOID lparam)
{
    ...
    while(1)
    {
        ...
        {
            buffer = L_MALLOC(ComStat.cbInQue);
...
{     
    ...
    dev->rxq.buf = L_MALLOC(COM_RX_BUF_SIZE);
    dev->rxq.size = COM_RX_BUF_SIZE;
    QueueClean(&dev->rxq);

    memset(&dev->txq, 0, sizeof(dev->txq));
    dev->txq.buf = L_MALLOC(COM_TX_BUF_SIZE);
