...
HTTP_SESSION_HANDLE  HTTPClientOpenRequest (HTTP_CLIENT_SESSION_FLAGS Flags)
{
    ...
    pHTTPSession = (P_HTTP_SESSION)tls_mem_alloc(HTTP_ALIGN(sizeof(HTTP_SESSION)));
    ...
    nAllocationSize = HTTP_ALIGN(nAllocationSize);
    // Allocate the headers buffer
    pHTTPSession->HttpHeaders.HeadersBuffer.pParam = (CHAR*)tls_mem_alloc(nAllocationSize);
    // Check the returned pointer
    if(!pHTTPSession->HttpHeaders.HeadersBuffer.pParam)
    {
        // tls_mem_alloc() error, free the containing structure and exit.
        tls_mem_free(pHTTPSession);
        return 0;
...
UINT32 HTTPClientCloseRequest (HTTP_SESSION_HANDLE *pSession)
{
    ...
    {
        TLS_DBGPRT_INFO("free pHTTPSession->HttpUrl.Url = %x\n", pHTTPSession->HttpUrl.Url);
                           ...
                           void *pReserved)
{
    ...
    case(AuthSchemaDigest):
        ...
        break;
#endif //TLS_CONFIG_HTTP_CLIENT_AUTH_DIGEST
                              ...
                              UINT32 nClientPort)   //  [IN] Client side port 0 for none
{
    ...
    do
    {
        ...
        if((pHTTPSession->HttpHeaders.HttpVerb == VerbPost || pHTTPSession->HttpHeaders.HttpVerb == VerbPut)&& TotalLength == TRUE)
        {
	     ...
	     if((nRetCode = HTTPIntrnHeadersAdd(pHTTPSession,"Content-Type",12,"application/x-www-form-urlencoded",33))!= HTTP_CLIENT_SUCCESS)
            {
                break;
            }
        }
                        ...
                        UINT32 nUrlLength)  // [IN] The length the Url string
{
    ...
    pHTTPSession->HttpUrl.Url = tls_mem_alloc(nUrlLength+2);
    if(pHTTPSession->HttpUrl.Url == NULL)
        return HTTP_CLIENT_ERROR_NO_MEMORY;
    //TLS_DBGPRT_INFO("HttpUrl.Url=%x, size=%d\n", pHTTPSession->HttpUrl.Url, nUrlLength+2);
                              ...
                              UINT32 nNewBufferSize)    // [IN] The new (and larger) buffer size
{
    ...
    nNewBufferSize=HTTP_ALIGN(nNewBufferSize);
    //TLS_DBGPRT_INFO("nNewBufferSize=%d\n", nNewBufferSize);
    pPtr = (CHAR*)tls_mem_alloc(nNewBufferSize);
    if(!pPtr)
    {
        // malloc() error
        return HTTP_CLIENT_ERROR_NO_MEMORY;
...
//    int              nNonBlocking    = 1;                // non blocking mode parameter
    ...
    do
    {
        ...
        */
        ...
        ServerAddress.sin_family        = OPENAT_AF_INET;
#ifdef _HTTP_BUILD_AMT
        ...
        {
            LoaclAddress.sin_family         = OPENAT_AF_INET;
#ifdef _HTTP_BUILD_AMT
		...
		if(OPENAT_EEXIST == nRetCode)
			nRetCode = 0;
        if(nRetCode == 0 || nRetCode == HTTP_EWOULDBLOCK)
        {      
        ...
        while(1)
        {
            ...
            if(FD_ISSET(pConnection->HttpSocket ,&pConnection->FDWrite))
            {
                ...
                if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SECURE) == HTTP_CLIENT_FLAG_SECURE)
                {
                    ...
                    {
                        nRetCode = HTTPWrapperSSLNegotiate((HTTP_SESSION_HANDLE)pHTTPSession,pConnection->HttpSocket,0,0,"desktop");
                        if(nRetCode != 0)
                        {
                            // TLS Error
                      ...
                      BOOL PeekOnly)      // [IN] State if we should only peek the socket (default is no)
{
    ...
    do
    {
        ...
        while(1)
        {
            ...
            if(nRetCode == SOCKET_SSL_MORE_DATA || FD_ISSET(pConnection->HttpSocket ,&pConnection->FDRead)) // Are there any read events on the socket ?
            {
                if(nRetCode != SOCKET_SSL_MORE_DATA){
                    // Clear the event 
                ...
                {     
                    ...
                    if((pHTTPSession->HttpFlags & HTTP_CLIENT_FLAG_SECURE) == HTTP_CLIENT_FLAG_SECURE)
                    {
                        if((nRetCode = HTTPWrapperSSLRecv(pHTTPSession->ssl,pConnection->HttpSocket,pData,*(nLength),0)) == SOCKET_ERROR)
                        {
                            // Socket error
                            nRetCode =  HTTP_CLIENT_ERROR_SOCKET_RECV;
...
static UINT32 HTTPIntrnGetRemoteHeaders (P_HTTP_SESSION pHTTPSession)
{
    ...
    do
    {
        ...
        while(nBytesRead > 0)
        {
            ...
            {
                TLS_DBGPRT_INFO("HTTPIntrnRecv nRetCode = %d\n", nRetCode);
                if(nRetCode != HTTP_CLIENT_ERROR_SOCKET_TIME_OUT)
                    nRetCode =  HTTP_CLIENT_ERROR_HEADER_RECV; // This was marked out for some reason
                break;
            }
			//TLS_DBGPRT_INFO("[http] %s pPtr: %02x  Got %d bytes", __func__, (pPtr-1)[0], pHTTPSession->HttpHeaders.HeadersIn.nLength);
        }
    }while(0);
    ...
    if(pHTTPSession->pDebug)
    {
        pHTTPSession->pDebug("HTTPIntrnGetRemoteHeaders",NULL,0,"Got %d bytes, nRetCode %d",
              pHTTPSession->HttpHeaders.HeadersIn.nLength, nRetCode);
    }
#endif
                             ...
                             UINT32 nOffset)         //  [IN]  Optionaly privide an offset to start looking from
{
    ...
    do
    {
        ...
        Header = tls_mem_alloc(HTTP_CLIENT_MAX_HEADER_SEARCH_CLUE);
	if(Header == NULL)
		return HTTP_CLIENT_ERROR_NO_MEMORY;
...
static UINT32 HTTPIntrnHeadersParse (P_HTTP_SESSION pHTTPSession)
{
    ...
    do
    {
        ...
        HTTPToken = tls_mem_alloc(HTTP_CLIENT_MAX_TOKEN_LENGTH);
	 if(HTTPToken == NULL)
		return HTTP_CLIENT_ERROR_NO_MEMORY;
        ...
        if(HTTPIntrnHeadersFind(pHTTPSession,"content-length",&HTTPParam,TRUE,0) == HTTP_CLIENT_SUCCESS)
        {
            ...
            if(pHTTPSession->pDebug)
            {
                pHTTPSession->pDebug("HTTPIntrnHeadersParse",NULL,
                    0,"HTTPParam.nLength %d", HTTPParam.nLength);
            }
#endif
        }
...
#endif
        Cred = tls_mem_alloc(HTTP_CLIENT_MAX_64_ENCODED_CRED/2);
        if(Cred == NULL)
		return HTTP_CLIENT_ERROR_NO_MEMORY;
        Cred64 = tls_mem_alloc(HTTP_CLIENT_MAX_64_ENCODED_CRED);
	 if(Cred64 == NULL)
	 {
	 	tls_mem_free(Cred);
		return HTTP_CLIENT_ERROR_NO_MEMORY;
